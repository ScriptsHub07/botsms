...
class SMSActivate  {
	...
	function getOperators ($country){
		...
		$this->setEndpoint ('https://api.sms-activate.org/stubs/handler_api.php');

		$request = $this->request ("act=countryChange&data={$country}", 'POST', [
			'authority: api.sms-activate.org',
			'accept: application/json, text/javascript, */*; q=0.01',
			'x-requested-with: XMLHttpRequest',
			'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36',
			'content-type: application/x-www-form-urlencoded; charset=UTF-8',
			'origin: https://api.sms-activate.org',
			'sec-fetch-site: same-origin',
			'sec-fetch-mode: cors',
			'sec-fetch-dest: empty',
			'referer: https://api.sms-activate.org/',
			'accept-language: pt-BR,pt;q=0.9,en-US;q=0.8,en;q=0.7'
		]);
